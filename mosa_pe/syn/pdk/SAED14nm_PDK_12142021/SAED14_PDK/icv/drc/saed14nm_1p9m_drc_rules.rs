...
/*************************  NWELL RULES ***************************/
	...
	{
		@ "NWELL.A.1 Minimum area must be 0.065";
		area(nwell, value < 0.065);
	}
...
*/
	{
		@ "DIFF_15.S.1 Minimum spacing must be 0.028"; 
...
//***********************************************   M1 RULES 
	...
	{
		@ "M1.W.1: Minimum M1 width must be 0.03"; 
		internal1 ( m1 , distance < 0.03 ,extension = RADIAL, relational = POINT_TOUCH );
	}
	{
...
//***********************************************   M2 RULES 
	...
	{
		@ "M2.W.1: Minimum M2 width must be 0.03"; 
		internal1 ( m2 , distance < 0.03 ,extension = RADIAL, relational = POINT_TOUCH );
	}
	{
...
//		length_edge(interacting_edge(m2, vertex_edge(m2, {> 90}), count == 2), < 0.055);
	...
	{	
		@ "M2.A.1  M2 minimum area must be 0.003"; 
		area ( m2 , value < 0.003 );
	}
	{
...
//***********************************************   M3 RULES 
	...
	{
                @ "M3.S.1.1: Minimum space for run length >= 10.7 must be 0.06";
                external1(m3 , distance < 0.06, extension = NONE,projection_length >= 10.7, orientation = PARALLEL, intersecting = {}, projection = IN, orthogonal = ALL,look_thru = ALL);
        }
...
//		length_edge(interacting_edge(m3, vertex_edge(m3, {> 90}), count == 2), < 0.055);
	...
	{
		@ "M3.S.2.1: Minimum spacing of m3 and m3_3 must be 0.024 "; 
	    	external2(m3_not_int_m3_3,m3_3_not_int_m3,distance < 0.024, extension = RADIAL,relational = POINT_TOUCH);
	}
	...
	{
		@ "M3.S.2: Minimum spacing if metal width of one of lines > 0.06 and  their parallel run length > 0.7 must be 0.06"; 
	    	external2(m3_w_006,m3,distance < 0.06, extension = NONE,projection_length > 0.7, orientation = PARALLEL, intersecting = {}, projection = IN, orthogonal = ALL,look_thru = ALL);
	}
...
//***********************************************   M4 RULES 
	...
	{
		@ "M4.W.1: Minimm M4 width must be 0.05"; 
		internal1 ( m4 , distance < 0.05 ,extension = RADIAL, relational = POINT_TOUCH );
...
/******************** CPO rules *******************************/
	...
	{
                @"CPO.W.2 Minimum horizontal width is 0.034";
                internal1 ( cpo, distance <0.034,extension=RADIAL, relational = POINT_TOUCH, direction = HORIZONTAL );
        }
	{
		@"CPO.W.3 Maximum horizontal width 78";
		correct = internal1 ( cpo, distance <=78,extension=RADIAL, relational = POINT_TOUCH, direction = HORIZONTAL );
		cpo not_interacting correct;
	}
...
//***************************************      VIA1 RULES     *****************************/
    ...
    via1mg  = rectangles (via1, {0.05,0.05});
    via1lg  = rectangles (via1, {0.06,0.06});
    via1bar = rectangles (via1, {0.03,0.06});
{
        @ "VIA1.W.1 VIA1.W.2 VIA1.W.3 VIA1.W.4: VIA1 must be square with 0.03, 0.05 or 0.06 width and length or VIA1 must be rectangular with length 0.06 and width 0.03";
        not(via1,(via1sq or via1mg or via1lg or via1bar));
}


{
        @ "VIA1.S.1: Minimum center to center spacing of VIA1SQ to VIA1SQ: 0.05";
        center_to_center1( via1sq , distance < 0.05 );
}
{
...
{
        @ "VIA1.B.2: VIA1 must be covered by (M1 or M1_3) and (M2 or M2_3), min. enclosure is 0.002um";
        //not ( via1 , m1 );
...
//***************************************      VIA2 RULES     *****************************/
    ...
    via2mg  = rectangles (via2, {0.05,0.05});
    via2lg  = rectangles (via2, {0.06,0.06});
    via2bar = rectangles (via2, {0.03,0.06});
{
        @ "VIA2.W.1 VIA2.W.2 VIA2.W.3 VIA2.W.4: VIA2 must be square with 0.03, 0.05 or 0.06 width and length or VIA2 must be rectangular with length 0.06 and width 0.03";
        not(via2,(via2sq or via2mg or via2lg or via2bar));
}
...
//***************************************      VIA3 RULES     *****************************/
    via3mg  = rectangles (via3, {0.046,0.046});
    via3sq  = rectangles (via3, {0.06,0.06});
    via3lg  = rectangles (via3, {0.12,0.12});
    via3bar = rectangles (via3, {0.12,0.06});
{
        @ "VIA3.W.1 VIA3.W.2 VIA3.W.3 VIA3.W.4: VIA3 must be square with 0.046, 0.06 or 0.12 width and length or VIA3 must be rectangular with length 0.12 and width 0.06";
        not(via3,(via3sq or via3lg or via3mg or via3bar));
}
